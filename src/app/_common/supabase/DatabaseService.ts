...
import {
  TABLE_PORTFOLIO_MODAL,
  PORTFOLIO_MODAL_STATUS_ACTIVE,
  RPC_ADD_VIEW_COUNT,
} from "@/app/_common/constants/supabase";
import { PortfolioModal } from "@/app/_common/supabase/types";

export class BaseDatabaseService<
  T extends SupabaseClient | Promise<SupabaseClient>,
> {
  ...
  async updatePortfolioViewCount(
    ...
    const { data, error } = await supabase.rpc(RPC_ADD_VIEW_COUNT, {
