...
export const SortableItem = ({ id, content, timestamp }: IToDoItem) => {
  const {
    attributes,
    listeners,
    setNodeRef,
  ...
  return (
    ...
    <div
      ref={setNodeRef}
      style={style}
      className={`touch-none w-full flex justify-between p-2 border-solid border-black border-2 rounded-sm ${isDragging ? "opacity-50" : "opacity-100"}`}
    >
      <Item id={id} content={content} timestamp={timestamp} />
